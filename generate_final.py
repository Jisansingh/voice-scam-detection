...
hindi_templates = {
    ...
    'banking': ["atm pin change karni hai", "net banking activate karni hai", "account statement download karni hai", "credit card bill pay karna hai", "loan ke liye apply karna hai", "fd interest rate dekhna hai", "debit card blocked ho gaya hai", "upi payment karni hai", "bank branch jaana padega", "credit score check karna hai"],
    ...
    'college': ["fest mein participate karna hai", "attendance maintain karni hai", "club meeting mein aana hai", "bus timing dekhni hai", "registration form fill karna hai", "library card renew karna hai", "mess menu change hua hai", "seminar mein attend karna hai", "sports trials hain", "cultural event ka registration karna hai"],
...
for cat in categories:
    texts = hinglish_templates[cat]
    for t in texts:
        rows.append({'text': t.lower(), 'label': '0', 'category': cat, 'language': 'hinglish', 'tactics': 'none'})
    while len([r for r in rows if r['category']==cat]) < 35:
        t = random.choice(texts)
        if t.lower() not in [r['text'] for r in rows]:
            rows.append({'text': t.lower(), 'label': '0', 'category': cat, 'language': 'hinglish', 'tactics': 'none'})

for cat in categories:
    texts = hindi_templates[cat]
    for t in texts:
        rows.append({'text': t.lower(), 'label': '0', 'category': cat, 'language': 'hi', 'tactics': 'none'})
    while len([r for r in rows if r['category']==cat]) < 50:
        t = random.choice(texts)
        if t.lower() not in [r['text'] for r in rows]:
            rows.append({'text': t.lower(), 'label': '0', 'category': cat, 'language': 'hi', 'tactics': 'none'})
